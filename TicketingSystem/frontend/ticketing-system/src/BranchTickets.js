...
const BranchTickets = () => {
    ...
    useEffect(() => {
        ...
        const fetchUserDetails = async () => {
            try {
                const response = await axios.get('http://192.168.10.245:8080/user/details', { withCredentials: true });
        ...
        const fetchTickets = async (branchCode) => {
            try {
                const response = await axios.get(`http://192.168.10.245:8080/branch/tickets?branchCode=${branchCode}`, { withCredentials: true });
                if (isMounted) {
        ...
        const getTicketsForBranch = async () => {
            try {
                const userDetails = await fetchUserDetails();
                const branchCode = userDetails.branchCode;
                await fetchTickets(branchCode);
            } catch (err) {
