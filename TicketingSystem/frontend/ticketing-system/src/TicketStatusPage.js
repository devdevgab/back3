...
const TicketStatusPage = () => {
    ...
    useEffect(() => {
        ...
        const fetchTickets = async () => {
            try {
                const response = await axios.get('http://192.168.10.245:8080/tickets/getTicketsResolves', { withCredentials: true });
                if (isMounted) {
                    ...
                    if (ticketsArray.length > 1) {
                        ticketsArray.sort((a, b) => a.ticketId - b.ticketId);
                        setTickets([...ticketsArray]); // Force React to update state
                    }
                }
            } catch (err) {
                if (isMounted) {
    ...
    const handleMarkClosed = async (ticketId) => {
        try {
            const response = await fetch(`http://192.168.10.245:8080/mark-closed/${ticketId}`, {
                method: 'PUT',
                credentials: 'include',
            });
            const updatedTicket = await response.json();
    ...
    const handleCloseMark = async () => {

        if (!selectedTicket) return;

        try{
            const updatedProgress = await handleMarkClosed(selectedTicket.ticketId);
    ...
    const handleMarkOpen = async (ticketId) => {
        try {
            const response = await fetch(`http://192.168.10.245:8080/mark-open/${ticketId}`, {
                method: 'PUT',
                credentials: 'include',
            });
            const updatedTicket = await response.json();
    ...
    const handleOpenMark = async () => {
        if (!selectedTicket) return;

        try{
            const updatedProgress =  await handleMarkOpen(selectedTicket.ticketId);
            if (updatedProgress) {
                setTickets(prevTickets =>
                    prevTickets.map(ticket =>
                        ticket.ticketResolved === updatedProgress.ticketResolved ? updatedProgress : ticket
    ...
    const handlePLSMark = async () => {
        if (!selectedTicket) return;

        try{
            const updatedProgress =  await handleMarkPLS(selectedTicket.ticketId);
            if (updatedProgress) {
                setTickets(prevTickets =>
                    prevTickets.map(ticket =>
                        ticket.ticketResolved === updatedProgress.ticketResolved ? updatedProgress : ticket
    ...
    const handleMarkPLS = async (ticketId) => {
        try {
            const response = await fetch(`http://192.168.10.245:8080/mark-pls/${ticketId}`, {
                method: 'PUT',
                credentials: 'include',
            });
            const updatedTicket = await response.json();
    ...
    const handleRemarks = async (ticketId) =>{
        
        setError('');
        setSuccess('');

        try{
            const response = await axios.put(`http://192.168.10.245:8080/ticket/remarks/${selectedTicket.ticketId}`, ticketData, { withCredentials: true });
        ...
        <Container component="main" maxWidth="lg">
                    ...
                    <Table style={{ width: '100%' }}>
                        <TableHead>
                            <TableRow>
                                ...
                                <StyledTableCell>Ticket Progress</StyledTableCell>
                        ...
                        <TableBody>
                            {selectedTicket && (
                                <>
                                    ...
                                    <TableRow>
                                        <DetailTableCell><strong>Remarks</strong></DetailTableCell>
                                        <DetailTableCell>{
                                            selectedTicket.ticketRemarks ===null || selectedTicket.ticketRemarks =="" ? "No Remarks" : selectedTicket.ticketRemarks
            ...
            <DialogContent>
                <TextField
                    ...
                    multiline
                    rows={4}
                    value={ticketData.TicketRemarks}
                    name="TicketRemarks"
                    onChange={(e) => setTicketData({ ...ticketData, TicketRemarks: e.target.value })}
                    placeholder="Enter your remarks here..."
                    sx={{ mt: 2, width: "30rem" }}
                />
            </DialogContent>
            <DialogActions>
                <Button color="secondary" onClick = {handleClose2ndModal}>
                    Cancel
                </Button>   
