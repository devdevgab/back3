...
const AcceptedTickets = () => {
    ...
    useEffect(() => {
        ...
        const fetchTickets = async () => {
            try {
                const response = await axios.get('http://192.168.10.245:8080/tickets/accepted', { withCredentials: true });
                setTickets(response.data); // Store the tickets in state
                console.log('Fetched Tickets:', response.data);
            } catch (err) {
                setError(err.response?.data?.message || 'Error fetching ticketssssss');
            }
        };
        ...
        <Container component="main" maxWidth="lg">
            <Paper elevation={6} style={{ padding: '16px', marginTop: '32px' }}>
                <Typography variant="h5" gutterBottom>
                    List of Accepted Tickets by ICT and BM 
                </Typography>
